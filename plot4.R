...
with(df, {
  ...
  #2
  with(df, plot(DateTime,Voltage, type="l", xlab="datetime", cex.lab=.8, cex.axis=.8))
  ...
  legend("topright", lty=1, lwd=1, col = c("black", "blue", "red"), legend = c("Sub_metering_1", 
                "Sub_metering_2", "Sub_metering_3"), cex=.75, bty="n")
  
  #4
  with(df, plot(DateTime, Global_reactive_power, type="l", xlab="datetime", cex.lab=.8, cex.axis=.8))
